...
// GET - Obtener productos desactualizados
export async function getOutdatedProducts({ page = 1, search = "" }: GetAllProductsParams = {}): Promise<GetAllProductsResponse> {
  try {
    const params = new URLSearchParams();
    if (page) params.set('page', page.toString());
    if (search) params.set('search', search);

    const response = await fetch(`${baseUrl}/api/products/outdated?${params.toString()}`, {
      cache: 'no-store'
    });
    const json: { products: ProductType[], total: number } = await response.json();
