...
import { OUTDATED_PRODUCTS_DAYS, PRODUCTS_PER_PAGE, formatProducts } from "@/utils";
...
export async function GET(request: Request) {
  try {
    ...
    const outdatedBefore = new Date();
    outdatedBefore.setDate(outdatedBefore.getDate() - OUTDATED_PRODUCTS_DAYS);
    ...
    const productsRaw = await prisma.$queryRaw<Array<ProductResponseType>>(Prisma.sql`
      ...
      WHERE COALESCE(p.updated_at, p.created_at) <= ${outdatedBefore}
        AND (
    ...
    console.log(error);
    return NextResponse.json(
      { message: "Error interno del servidor al obtener productos desactualizados" },
      { status: 500 }
    );
  }
}
